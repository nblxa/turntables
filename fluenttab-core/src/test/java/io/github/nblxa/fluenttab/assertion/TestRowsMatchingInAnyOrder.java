...
import static io.github.nblxa.fluenttab.FluentTab.test;
...
public class TestRowsMatchingInAnyOrder {
  ...
  private AssertionProxy.Builder matchInAnyOrder(Tab expected, Tab actual) {
    return AssertionProxy.builder()
        .expected(expected)
        .actual(actual)
        .rowMode(FluentTab.RowMode.MATCHES_IN_ANY_ORDER);
  }
  ...
  public void matchesInSameOrder() {
    Tab expected = FluentTab.tab().row(1, 2).row(5, 6);
    Tab actual = FluentTab.tab().row(1, 2).row(5, 6);
  ...
  public void empty_doesntMatchNull() {
    Tab actual = FluentTab.tab().row(null);
    Tab expected = FluentTab.tab();
  ...
  public void withAssertions_oneActualMatchesBothExpected_theOtherDoesnt_fails() {
    ...
    Tab expected = FluentTab.tab().row(test, test).row(test, test);
    Tab actual = FluentTab.tab().row(2, 1).row(3, 3);
  ...
  public void withAssertions_onlyOneActual_matchingBothExpected() {
    IntPredicate test = testInt(i -> i == 1 || i == 2);
    Tab expected = FluentTab.tab().row(test, test).row(test, test);
    Tab actual = FluentTab.tab().row(2, 1);
  ...
  public void withAssertions_twoEqualActuals_matchingOneOfTwoExpected() {
    IntPredicate test = testInt(i -> i == 1 || i == 2);
    Tab expected = FluentTab.tab().row(test, test).row(0, test);
    Tab actual = FluentTab.tab().row(1, 2).row(1, 2);
  ...
  public void assertionsOrderCanCauseMismatch_assumingActualSortedDesc_matches() {
    Tab expected = FluentTab.tab().row(test(o -> true)).row(testInt(i -> i == 100));
    Tab actual = FluentTab.tab().row(100).row(2);
  ...
  public void assertionsOrderCanCauseMismatch_assumingActualSortedAsc_matches() {
    Tab expected = FluentTab.tab().row(test(o -> true)).row(testInt(i -> i == 2));
    Tab actual = FluentTab.tab().row(2).row(100);
